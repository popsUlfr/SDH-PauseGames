...
const Item: VFC<{ serverAPI: ServerAPI, app: AppObject }> = ({serverAPI, app}) => {
  ...
  return (
    <ToggleField checked={isPaused}
      ...
      icon={((app.icon_data && app.icon_data_format) || app.icon_hash) ? <img style={{maxWidth: 32, maxHeight: 32}} src={(app.icon_data) ? ("data:image/" + app.icon_data_format + ";base64," + app.icon_data) : ("/assets/" + app.appid + "_icon.jpg?v=" + app.icon_hash) } /> : null}
...
const Content: VFC<{ serverAPI: ServerAPI }> = ({serverAPI}) => {
  const [runningApps, setRunningApps] = useState<AppObject[]>([]);

  useEffect(() => {
    const refresh = async (app: AppLifetimeObject) => {
      await sleep(100); // wait for a bit before going on
      const runningAppsTmp: AppObject[] = Router.RunningApps;
      const toKeep = await Promise.all(runningAppsTmp.map((async (a) => {
        ...
        if (!a.instanceid && app.nInstanceID !== 0 && a.appid !== 0 && app.unAppID !== 0 && a.appid === app.unAppID) {
          a.instanceid = app.nInstanceID;
        }
        if (!a.instanceid && a.appid !== 0) {
          // we may need to retry to get the pid since the process is still getting started
          for (let i = 0; i < 3; ++i) {
            const ret = await serverAPI.callPluginMethod<{appid: number}, number>("pid_from_appid", {appid: a.appid});
      ...
      const newRunningAppsTmp: AppObject[] = [];
